    ...
    */
    ...
    selectQuery: function(tableName, options, factory) {
      ...
      var table               = null
        ...
        , subQuery            = (limit || options.offset) && (options.hasIncludeWhere || options.hasIncludeRequired || options.hasMultiAssociation)
        , subQueryItems       = []
      ...
      if (subQuery && mainAttributes) {
        if (factory.hasPrimaryKeys) {
          ...
          if (mainAttributes.indexOf("id") == -1){
      ...
      if (options.include) {
        ...
        var addWhere = function(where, key, value) {
          if (where instanceof Utils.and || where instanceof Utils.or) {
            var subWhere = {};
            subWhere[key] = value;
            return new Utils.and([where, subWhere]);
          } else {
            where[key] = value;
            return where;
          }
        ...
        var generateJoinQueries = function(include, parentTable) {
          ...
          if (options.includeIgnoreAttributes !== false && !include.isExclude) {
          ...
          if (through) {
            ...
            if (include.where) {
              ...
              if (subQuery) {
                ...
                _where += ")";
                _where += include.isExclude ? " IS NULL" : " IS NOT NULL"

                options.where = addWhere(options.where, "__"+throughAs, self.sequelize.asIs(_where));
              } else if (include.isExclude) {
                if (!options.where) options.where = {};

                //for isExcludes we're using LEFT OUTER JOIN - add (target primary key) is NULL to outer where
                options.where = addWhere(options.where, "__"+as, self.sequelize.asIs(
                  self.quoteIdentifier(as) + "." +
                  self.quoteIdentifier(attrTarget) +
                  ' IS NULL'
                ));
              }
            }
            ...
            var primaryKeysLeft = ((association.associationType === 'BelongsTo') ? Object.keys(association.target.primaryKeys) : Object.keys(include.association.source.primaryKeys))
              ,primaryKeysRight = ((association.associationType === 'BelongsTo') ? Object.keys(include.association.source.primaryKeys) : Object.keys(association.target.primaryKeys))
              ...
              , joinOn
              , fullAttrRight   = self.quoteIdentifier(tableRight) + "." + self.quoteIdentifier(attrRight)
            ...
            if (subQuery && !include.subQuery && include.parent.subQuery) {
              ...
              joinOn = self.quoteIdentifier(tableLeft) + "." + self.quoteIdentifier(attrLeft) + " = "
            }
            joinOn += fullAttrRight

            if (
              subQuery &&
              include.parent.table === self.quoteIdentifier(parentTable) &&
              tableRight === parentTable &&
              subQueryAttributes.indexOf(fullAttrRight) < 0 &&
              subQueryAttributes.indexOf(self.quoteIdentifier(tableRight) + '.*') < 0
              ) {
              subQueryAttributes = subQueryAttributes.concat([fullAttrRight]);
            }
            if (include.where) {
              joinOn += " AND "+self.getWhereConditions(include.where, self.sequelize.literal(self.quoteIdentifier(as)), include.daoFactory, whereOptions)
              ...
              if (subQuery && association.isMultiAssociation) {
                ...
                ')', include.isExclude ? 'IS NULL' : 'IS NOT NULL'].join(' ')))
              } else if (include.isExclude) {
                if (!options.where) options.where = {};

                //for isExcludes we're using LEFT OUTER JOIN - add (join primary key) is NULL to outer where
                options.where = addWhere(options.where, "__"+as, self.sequelize.asIs(
                  self.quoteIdentifier(as) + "." +
                  self.quoteIdentifier(((primaryKeysRight.length !== 1) ? 'id' : primaryKeysRight[0])) +
                  ' IS NULL'
                ));
              }
            }
            ...
            // Generate join SQL
            joinQueryItem += joinType + self.quoteIdentifier(table) + " AS " + self.quoteIdentifier(as) + " ON " + joinOn
