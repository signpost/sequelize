...
module.exports = (function() {
 ...
 ;(function() {
  ...
  DAOFactory.prototype.init = function(daoFactoryManager) {
    ...
    Utils._.each(this.rawAttributes, function(definition, attributeName) {
      if (definition.primaryKey) {
        self.primaryKeys[attributeName] = definition.type.toString()
      }
    })
  ...
  DAOFactory.prototype.findAll = function(options, queryOptions) {
    ...
    if (typeof options === 'object') {
      if (
            (options.hasOwnProperty('include') && options.include) ||
            (options.hasOwnProperty('exclude') && options.exclude)
          ) {
        hasJoin = true
  ...
  * @param {Array} exclude A list of associations which shall be excluded { include: [ DaoFactory1, DaoFactory2, ...] } or { include: [ { daoFactory: DaoFactory1, as: 'Alias' } ] }.
  ...
  */
  ...
  var validateIncludedElements = function(options, tableNames) {
    ...
    //"excludes" are basically "includes" with a flag
    if (options.exclude) {
      if (!Array.isArray(options.exclude)) {
        options.exclude = [options.exclude];
      }
      options.include = options.include.concat(options.exclude.map(function(exclude) {
    ...
    for (var index = 0; index < includes.length; index++) {
      ...
      include.subQuery = !include.association.isMultiAssociation && (include.hasIncludeRequired || include.required || include.isExclude)
  ...
  var validateIncludedElement = function(include, tableNames) {
    ...
    if (include.hasOwnProperty('attributes')) {
      ...
      primaryKeys.forEach(function(primaryKey) {
        if (include.attributes.indexOf(primaryKey) < 0) {
          include.attributes = include.attributes.concat([primaryKey])
        }
      });
    } else {
